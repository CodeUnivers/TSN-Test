...
exports.create = (req, res) => {
    if (!req.body.productId) {
        ...
        res.status(400).send({ message: "Choose Kind of Product between Male or Female!" });
        return;
...
exports.findAllByUserId = (req, res) => {
    ...
    Cart.findBy({ userId: userId }).then(data => {
        res.send(data);
    }).catch(err => {
        res.status(500).send({
            message: err.message || "Some error occurred while retrieving data Cart."
        });
    });
};

exports.countCartByUserId = (req, res) => {
    const userId = req.query.userId;
    const ct = Cart.find({ userId }).countDocuments();
    console.log(userId);
};